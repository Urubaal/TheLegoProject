...
# LEGO Purchase Suggestion System - Production Startup Script
# Optimized for production deployment

set -e

echo "🚀 Starting LEGO Purchase Suggestion System (Production Mode)"
echo "=============================================================="
...
if [ -z "$JWT_SECRET" ]; then
    print_warning "JWT_SECRET not set. Using default (NOT SECURE FOR PRODUCTION!)"
...
docker-compose exec -T database psql -U lego_user -d lego_purchase_system -f /docker-entrypoint-initdb.d/02-optimizations.sql > /dev/null 2>&1 || true
...
if docker-compose exec -T database pg_isready -U lego_user -d lego_purchase_system > /dev/null 2>&1; then
...
if docker-compose exec -T redis redis-cli ping | grep -q "PONG"; then
    ...
    print_warning "Redis health check failed (non-critical)"
fi
